package com.haizhang.dao;

import com.haizhang.uitil.jdbcUtil;
import org.apache.commons.dbutils.QueryRunner;
import org.apache.commons.dbutils.handlers.BeanHandler;
import org.apache.commons.dbutils.handlers.BeanListHandler;
import org.apache.commons.dbutils.handlers.ScalarHandler;
import java.lang.reflect.ParameterizedType;
import java.lang.reflect.Type;
import java.sql.Connection;
import java.util.List;




/**
 * @author Austin
 * @date 2018/12/1
 * 这是一个dao层里的基本类，在于具体的dao类，uderDao去继承它来用，不能new BaseDao()来直接调用
 * param<T>针对要操作各张数据表映射到java工程里java类，User
 */
public class BaseDao<T> {
    QueryRunner queryRunner = new QueryRunner();
    private Class<T> clazz;
    public BaseDao(){
        //用baseDao的构造方法初始化clazz属性
        Type superType = this.getClass().getGenericSuperclass();//拿到调用者的父类的类型
        if(superType instanceof ParameterizedType){   //parameterizedType表示参数化类型
            ParameterizedType pt = (ParameterizedType) superType;
            Type[] tarry = pt.getActualTypeArguments(); //返回一个类型数组，第一个元素就是的T
            if(tarry[0] instanceof Class){
                clazz = (Class<T>) tarry[0];
            }
        }
    }
    //对数据库有增删改查

    /*
     * 查询数据表，取出sql语句的结果集的第一条数据，封装成一个类的对象返回，不支持事务
     *用到dbutils工具类
     *null的位置 ,应该传入BaseDao<T>里边的T的真正用的时候的类型的Class
     */

    public T get(String sql,Object...args){

        T entity = null;
        try {
            Connection conn = jdbcUtil.getDataSource().getConnection();
            entity = queryRunner.query(conn,sql,new BeanHandler<T>(clazz),args);//BeanHandler<T>将结果值封装成一个对象
        }catch (Exception e){
            e.printStackTrace();
        }
        return entity;
    }
    /*
     * 获得多条记录的通用方法,用泛型才能实现通用
     */
    public List<T> getList(String sql,Object...args){

        List<T> list = null;
        try {
            Connection conn = jdbcUtil.getDataSource().getConnection();
            list = queryRunner.query(conn,sql,new BeanListHandler<T>(clazz),args);
        }catch (Exception e){
            e.printStackTrace();
        }
        return list;
    }

    /*
     * 实现insert，update，delete通用的更新方法
     */
    public int update(String sql, Object...args){

        int row = 0;
        try {
            Connection conn = jdbcUtil.getDataSource().getConnection();
            row = queryRunner.update(conn,sql,args);
        }catch (Exception e){
            e.printStackTrace();
        }
        return row;
    }

    /*
     * 通用的放回sql语句的结果只有一个数值的类型大的查询，用户个数count(id)
     */
    public Object getValue(String sql, Object... args) {

        Object object = null;
        try {
            //拿conn
            Connection conn = jdbcUtil.getDataSource().getConnection();
            object = queryRunner.query(conn, sql, new ScalarHandler(),args);
        }catch(Exception e) {
            e.printStackTrace();
        }
        return object;
    }

}
